...
class PartOne < Service
  const :calibrations, String

  def call
    sum = 0
    calibrations.each_line do |line|
      line = line.strip
      digits = line.gsub(/[^\d]/, '')
      log(line: line, digits: digits)

      candidate = digits[0] + digits[-1]
...
[:found, "eightseven2threethreenineqtwonezkq", "eight", 0, 5]

So this is annoying. We're doing too much work!

New algorithm:

1. find the _first_ digit, !!!___walking forwards___!!!
2. find the _last_ digit, !!!___walking backwards___!!!
3. construct the number & sum

=end

class PartTwo < Service
  VALID_DIGITS = %w[one two three four five six seven eight nine]
  VALID_REGEX = /#{VALID_DIGITS.join("|")}/
  ...
  def find_first_digit(line)
    find_digit(line, :forwards)
  end

  def find_last_digit(line)
    find_digit(line, :backwards)
  end

  def find_digit(line, mode)
    if mode == :backwards
      line = line.dup.reverse
    end
    ...
    while cur < line.length
      ...
      (3..5).each do |i|
        ...
        candidate = line[start_i...end_i]
        if mode == :backwards
          candidate = candidate.reverse
        end

        VALID_DIGITS.each_with_index do |digit, d|
          if candidate == digit
            return (d + 1).to_s
          end
        end
...
class Day1Test < Minitest::Test
  ...
  def test_part_1_example
    ...
    assert_equal(12 + 38 + 15 + 77, PartOne.call(calibrations: example))
  end

  def test_part_1_real
    assert 55971, PartOne.call(calibrations: INPUT)
  end
  ...
  def test_part_2_example
    ...
    assert_equal(29 + 83 + 13 + 24 + 42 + 14 + 76, PartTwo.call(calibrations: example))
  end

  def test_part_2_real
    assert_equal(54719, PartTwo.call(calibrations: INPUT))
  end
