...
class Game < Value
  const :game_id, T.nilable(Integer)
  const :red, Integer
  const :green, Integer
  const :blue, Integer
  ...
  def self.parse(line)
    ...
    game_id = game.strip.split(/\s/, 2).last.to_i

    red = green = blue = 0

    rest.split(';').each do |round|
      round.split(',').each do |part|
        log(part: part.strip)
        count, color = part.strip.split(/\s/)
        count = count.to_i
        log(count: count, color: color)
  ...
  def possible?(test_game)
    return false if red > test_game.red
    return false if green > test_game.green
    return false if blue > test_game.blue
    return true
  end
...
class Day2Test < Minitest::Test
  ...
  def test_part_1_real
    assert_equal(2348, GameSet.call(input: INPUT, test_game: EXAMPLE_GAME))
  end

  def test_part_2_example
    assert_equal(2286, GamePower.call(input: EXAMPLE))
  end

  def test_part_2_real
    assert_equal(76008, GamePower.call(input: INPUT))
  end

  EXAMPLE_GAME = Game.new(red: 12, green: 13, blue: 14)
